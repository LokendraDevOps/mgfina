...
const LoanDetail = () => {
  ...
  return <>
    ...
    <Section className="-mt-8 relative z-10 !pt-0"><div className="grid gap-4 rounded-3xl border border-slate-200 bg-white p-6 text-slate-900 shadow-xl sm:grid-cols-3"><div><p className="text-sm text-slate-500">Interest rate from</p><strong className="mt-1 block text-2xl">{loan.rate ? `${loan.rate}% p.a.` : 'Card-specific'}</strong></div><div><p className="text-sm text-slate-500">Loan amount</p><strong className="mt-1 block text-2xl">{loan.maxAmount}</strong></div><div><p className="text-sm text-slate-500">Tenure</p><strong className="mt-1 block text-2xl">{loan.tenure}</strong></div></div></Section>
