...
const DebtConsolidationLoan = () => {
  ...
  return (
    <div className="overflow-hidden bg-[#040814] text-white">
      <div className="relative isolate">
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          ...
          {['card', 'emi', 'bank', 'graph', 'chart'].map((label, index) => (
            <motion.div
              key={label}
              className="absolute rounded-[1.7rem] border border-white/10 bg-white/6 px-4 py-3 text-xs uppercase tracking-[0.3em] text-slate-200 backdrop-blur-2xl"
          ...
          <div className="flex flex-col justify-center">
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <h1 className="mt-8 max-w-4xl text-5xl font-semibold tracking-[-0.05em] text-white sm:text-7xl">
                ...
                <span className="mt-4 block text-transparent bg-gradient-to-r from-cyan-300 via-white to-violet-300 bg-clip-text">
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                <Button className="rounded-full bg-white px-6 py-4 text-sm font-semibold text-slate-950 hover:bg-slate-100">Check My Eligibility <FiArrowRight /></Button>
                <Button variant="secondary" className="rounded-full border border-white/15 bg-white/5 px-6 py-4 text-sm font-semibold text-white hover:bg-white/10">Calculate My New EMI</Button>
              </div>
      ...
      <Section eyebrow="AI Analysis" title="Interactive financial dashboard" subtitle="A live-style snapshot of debt pressure, affordability, and future savings." className="py-24">
              ...
              {metrics.map(item => (
                <div key={item.label} className="rounded-3xl border border-white/10 bg-slate-950/55 p-4">
                  ...
                  <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10">
                    <motion.div initial={{ width: 0 }} whileInView={{ width: `${item.value}%` }} viewport={{ once: true }} transition={{ duration: 1, ease: 'easeOut' }} className="h-full rounded-full bg-gradient-to-r from-cyan-400 via-sky-400 to-violet-400" />
                  </div>
                </div>
              ))}
            </div>
                ...
                {[42, 58, 66, 78, 92, 100].map((bar, index) => (
                  <div key={index} className="flex-1">
                    <div className="flex h-44 items-end rounded-3xl border border-white/10 bg-slate-950/55 p-3">
                      <motion.div initial={{ height: 0 }} whileInView={{ height: `${bar}%` }} viewport={{ once: true }} transition={{ duration: 1 }} className="w-full rounded-2xl bg-gradient-to-t from-cyan-500 to-violet-500" />
                    </div>
                  </div>
                ))}
              </div>
      ...
      <Section eyebrow="Who Should Apply?" title="Designed for people managing multiple commitments" className="bg-[#071022] py-24">
        <div className="grid gap-4 lg:grid-cols-4">
          {[
            ...
            <div key={label} className="rounded-[1.5rem] border border-white/10 bg-white/5 p-6 text-center">
