...
export const BankCard = ({ bank }) => <motion.article whileHover={{ y: -5 }} className="surface-card p-6"><div className="flex items-center justify-between gap-4"><span className="flex h-14 w-32 items-center justify-center rounded-2xl border border-slate-200 bg-white px-3"><img src={bank.logo} alt={bank.logoAlt || bank.name} className="max-h-9 w-auto object-contain grayscale transition duration-300 hover:grayscale-0" loading="lazy"/></span><span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-700">★ {bank.rating}</span></div><h3 className="mt-5 text-xl font-bold text-slate-950">{bank.name}</h3><p className="mt-2 text-sm text-slate-600">{bank.benefit}</p><div className="mt-5 flex flex-wrap gap-2">{bank.products.map(p => <span key={p} className="rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-600">{p}</span>)}</div><Button as="a" href="/apply" className="mt-6 w-full">Explore offers</Button></motion.article>;
