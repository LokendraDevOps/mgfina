...
export const BankCard = ({ bank }) => <motion.article whileHover={{ y: -5 }} className="surface-card p-6"><div className="flex items-center justify-between"><span className="grid h-14 w-14 place-items-center rounded-2xl bg-slate-900 text-xs font-black text-white">{bank.short}</span><span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-700">★ {bank.rating}</span></div><h3 className="mt-5 text-xl font-bold">{bank.name}</h3><p className="mt-2 text-sm text-slate-500">{bank.benefit}</p><div className="mt-5 flex flex-wrap gap-2">{bank.products.map(p => <span key={p} className="rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-600">{p}</span>)}</div><Button as="a" href="/apply" className="mt-6 w-full">Explore offers</Button></motion.article>;
